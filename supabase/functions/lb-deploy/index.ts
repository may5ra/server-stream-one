...
function generateNginxConfig(streams: Stream[], lbPort: number, supabaseUrl: string, supabaseKey: string): string {
  // Build case-insensitive map entries for stream routing
  const mapEntries: string[] = [];
  
  streams.forEach((stream) => {
    const streamName = stream.name;
    const normalizedName = streamName.toLowerCase().replace(/[^a-z0-9]/g, '');
    const inputUrl = stream.input_url;
    
    // Parse the input URL to get host and path
    try {
      const url = new URL(inputUrl);
      // Add both original name and normalized version for matching
      mapEntries.push(`    "~*^${escapeRegex(streamName)}$" "${inputUrl}";`);
      // Also add normalized version
      mapEntries.push(`    "~*^${normalizedName}$" "${inputUrl}";`);
    } catch (e) {
      console.error(`Invalid URL for stream ${stream.name}: ${inputUrl}`);
    }
  });
...
# Stream URL mapping (case-insensitive regex)
map $stream_name $stream_backend_url {
    default "";
${mapEntries.join('\n')}
}

# Extract host from backend URL for proper proxying
map $stream_backend_url $backend_host {
    default "";
    "~^https?://([^/]+)" $1;
}
...
server {
    ...
    # Auth subrequest endpoint - validates username/password against Supabase
    location = /_auth {
        internal;
        proxy_pass ${supabaseUrl}/rest/v1/streaming_users?select=id,status,expiry_date&username=eq.$arg_username&password=eq.$arg_password&status=eq.active;
    ...
    location ~ ^/live/(.+)\\.ts$ {
        ...
        # Check if stream exists in map
        if ($stream_backend_url = "") {
            return 404 '{"error": "Stream not found", "requested": "$stream_name"}';
        }
        
        # Proxy to the backend URL with proper HLS handling
        ...
        # Critical for HLS streaming - disable buffering
        proxy_buffering off;
        proxy_cache off;
        proxy_request_buffering off;
        
        # Long timeouts for live streams
        proxy_connect_timeout 10s;
        proxy_read_timeout 86400s;
        proxy_send_timeout 86400s;
        
        # Pass through chunked encoding
