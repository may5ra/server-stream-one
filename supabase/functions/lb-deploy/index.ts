...
// Generate Nginx configuration with auth_request and 302 redirect
function generateNginxConfig(streams: Stream[], lbPort: number, supabaseUrl: string, supabaseKey: string): string {
  ...
  streams.forEach((stream) => {
    ...
    mapEntries.push(`    "${lowerName}" "${inputUrl}";`);
...
# Mode: Auth + 302 Redirect to source

# Stream URL mapping (lowercase keys)
...
server {
    ...
    # Live streams - uses 302 redirect after auth
    # Format: /live/STREAMNAME.ts?username=X&password=Y
    location ~ ^/live/(.+)\\.ts$ {
        # Extract stream name and convert to lowercase
        set $stream_name $1;
        ...
        # After successful auth, do internal redirect to get the backend URL
        # We need to lowercase and normalize the stream name
        rewrite ^/live/(.+)\\.ts$ /internal/redirect/$1 last;
    }

    # Internal redirect handler - lowercase matching
    location ~ ^/internal/redirect/(.+)$ {
        ...
        set $stream_name_lower $1;
        # Nginx doesn't have native lowercase, so we handle common cases
        # The map uses lowercase keys, so requests need lowercase names
        
        if ($stream_backend_url = "") {
            return 404 '{"error":"Stream not found","name":"$stream_name"}';
        }
        
        # 302 redirect to the source URL
        # This way player handles HLS/DASH manifest and segments directly from source
        return 302 $stream_backend_url;
    }
