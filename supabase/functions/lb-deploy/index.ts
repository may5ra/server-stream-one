...
function generateNginxConfig(streams: Stream[], lbPort: number, supabaseUrl: string, supabaseKey: string): string {
  // Build upstream blocks and map for stream routing
  ...
  streams.forEach((stream, index) => {
    ...
    try {
      ...
      const upstreamName = `stream_${index}`;
...
upstream ${upstreamName} {
    server ${url.host};
      ...
      // Add to map - store the full URL for rewrite
      mapEntries.push(`    "${streamName}" "${inputUrl}";`);
    } catch (e) {
...
# Username/password from query string
