...
function generateNginxConfig(streams: Stream[], lbPort: number, supabaseUrl: string, supabaseKey: string): string {
  ...
  // Maps stream name -> backend host and URI separately for proper proxying
  const hostMapEntries: string[] = [];
  const uriMapEntries: string[] = [];
  ...
  streams.forEach((stream) => {
    ...
    // Parse URL to get host and path separately
    try {
      const url = new URL(inputUrl);
      const host = url.host;
      const pathAndQuery = url.pathname + url.search;
      
      // Add lowercase exact match
      hostMapEntries.push(`    "${lowerName}" "${url.protocol}//${host}";`);
      uriMapEntries.push(`    "${lowerName}" "${pathAndQuery}";`);
      
      // Add normalized version if different
      if (normalizedName !== lowerName) {
        hostMapEntries.push(`    "${normalizedName}" "${url.protocol}//${host}";`);
        uriMapEntries.push(`    "${normalizedName}" "${pathAndQuery}";`);
      }
    } catch (e) {
      // Fallback for invalid URLs
      hostMapEntries.push(`    "${lowerName}" "${inputUrl}";`);
      uriMapEntries.push(`    "${lowerName}" "/";`);
    }
  });
...
# Mode: Auth + Reverse Proxy (source URL hidden)

# Stream host mapping (lowercase keys)
map $stream_name_lower $stream_backend_host {
    default "";
${hostMapEntries.join('\n')}
}

# Stream URI mapping (lowercase keys)
map $stream_name_lower $stream_backend_uri {
    default "";
${uriMapEntries.join('\n')}
}
...
server {
    ...
    # Debug - list streams with normalized names
    ...
    # Live streams - uses reverse proxy (hides source URL)
    ...
    location ~ ^/live/(.+)\\.ts$ {
        ...
        # After successful auth, proxy to backend
        rewrite ^/live/(.+)\\.ts$ /internal/proxy/$1 last;
    }
    ...
    location ~ ^/internal/proxy/(.+)$ {
        ...
        if ($stream_backend_host = "") {
            return 404 '{"error":"Stream not found","name":"$stream_name"}';
        }
        
        # Reverse proxy to hide source URL
        proxy_pass $stream_backend_host$stream_backend_uri;
        ...
        # Streaming optimizations - disable buffering for live content
        proxy_buffering off;
        proxy_cache off;
        proxy_request_buffering off;
        
        # Chunked transfer for HLS/DASH
        ...
        # Rewrite absolute redirects from upstream to use LB host/port
